...
import { BufferGeometry, InstancedMesh, type Material, MeshStandardMaterial, Mesh, Object3D } from 'three'
...
useGLTF.preload('/models/wandering_trader.glb')

const POOL_NAME = 'wanderingTraders'
const MAX_DENSITY = 0.02
const ZONE_DEFAULTS = { width: 20, forwardDepth: 30, backDepth: 30 }
...
function useWanderingTraderSubMeshes(): SubMesh[] {
  const { scene } = useGLTF('/models/wandering_trader.glb')
  return useMemo(() => {
    const out: SubMesh[] = []
...
export default function WanderingTraderScatter() {
  const subMeshes = useWanderingTraderSubMeshes()
  const defaults = useScatterDefaults('wanderingTraders')
  const envLabel = useEnvLabel()

  const pool = useControls(envLabel, {
    Tiles: folder(
      {
        'Wandering Traders': folder(
          {
  ...
  const model = useControls(envLabel, {
    Models: folder(
      { 'Wandering Trader': folder(modelControlsSchema(defaults.model), { collapsed: true }) },
      { collapsed: true }
    ),
  })

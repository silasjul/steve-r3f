...
useGLTF.preload('/models/desert_temple.glb')

const POOL_NAME = 'temples'
const MAX_DENSITY = 0.02
// Slider ceiling for the temple footprint — far above the 4-unit plant default
// so the configured no-spawn / spacing extent isn't clamped by leva.
const FOOTPRINT_MAX = 80
const ZONE_DEFAULTS = { width: 65, forwardDepth: 40, backDepth: 40 }
const CAPACITY = Math.max(4, Math.ceil(getTileCountRect(ZONE_DEFAULTS.width, ZONE_DEFAULTS.forwardDepth, ZONE_DEFAULTS.backDepth) * MAX_DENSITY))
...
function useTempleSubMeshes(): SubMesh[] {
  const { scene } = useGLTF('/models/desert_temple.glb')
  return useMemo(() => {
    const out: SubMesh[] = []
...
export default function TempleScatter() {
  const subMeshes = useTempleSubMeshes()
  const defaults = useScatterDefaults('temples')
    ...
    Tiles: folder(
      {
        Temples: folder(
          {
            ...poolControlsSchema(defaults.pool, { footprintMax: FOOTPRINT_MAX }),
            corridorClearance: {
              value: defaults.corridorClearance,
              min: 0,
              max: 40,
              step: 0.5,
              label: 'Corridor Clearance',
            },
            gap: { value: defaults.gap, min: 0, max: 4, step: 0.1, label: 'Gap (× footprint)' },
  ...
  const model = useControls(envLabel, {
    Models: folder(
      { Temple: folder(modelControlsSchema(defaults.model), { collapsed: true }) },
      { collapsed: true }
    ),
  })
  ...
  useScatterPool({
    ...
    walkCorridorClearance: pool.corridorClearance as number,
    ...
    variantCount: 1,
    fanAllMeshes: true,
    selfAvoidFactor: pool.gap as number,
